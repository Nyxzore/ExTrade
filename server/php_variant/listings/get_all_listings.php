...
if ($result) {
    while ($row = pg_fetch_assoc($result)) {
        ...
        $listings[] = [
            ...
            "price" => $row['price'] ? "R " . number_format($row['price'], 2) : "Contact",
    ...
    if (!empty($listings)) {
        ...
        log_impressions($db, $user_id, $ids, 'listing_impressions');
    }

    send_response("success", "Listings fetched", ["listings" => $listings]);
} else {
    send_error("Failed to fetch listings");
}
